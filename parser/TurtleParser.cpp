...
//---------------------------------------------------------------------------
TurtleParser::Exception::Exception(std::string message)
...
//---------------------------------------------------------------------------
TurtleParser::Lexer::Lexer(std::istream &in)
        : in(in), putBack(Token_Eof), line(1), readBufferStart(nullptr), readBufferEnd(nullptr)
// Constructor
{
}
...
//---------------------------------------------------------------------------
static std::string encodeUtf8(unsigned code)
...
// Lex a long std::string, first """ already consumed
{
    char c;
    while (read(c)) {
        if (c == '\"') {
    ...
    std::stringstream msg;
    msg << "lexer error in line " << line << ": invalid std::string";
    throw Exception(msg.str());
}
...
//---------------------------------------------------------------------------
TurtleParser::TurtleParser(std::istream &in)
...
//---------------------------------------------------------------------------
void TurtleParser::parseError(const std::string &message)
// Report an error
{
    std::stringstream msg;
...
{
    return (token == Lexer::Token_Name) || (token == Lexer::Token_A) || (token == Lexer::Token_True) ||
           (token == Lexer::Token_False);
}

//---------------------------------------------------------------------------
void TurtleParser::parseQualifiedName(const std::string &prefix, std::string &name)
// Parse a qualified name
{
    ...
    std::string expandedPrefix = prefixes[prefix];
    ...
    switch (token) {
        ...
        case Lexer::Token_LBracket: {
            ...
            if (token != Lexer::Token_RBracket) {
                ...
                triples.emplace_back(entry, predicate, object, objectType, objectSubType);
        ...
        case Lexer::Token_LParen: {
            // Collection
            std::vector<std::string> entries, entrySubTypes;
            std::vector<Type::Type_ID> entryTypes;
            while ((token = lexer.next()) != Lexer::Token_RParen) {
                lexer.ungetIgnored(token);
                entries.emplace_back();
                entryTypes.push_back(Type::Type_URI);
                entrySubTypes.emplace_back();
            ...
            nodes.emplace_back("http://www.w3.org/1999/02/22-rdf-syntax-ns#nil");

            // Derive triples
            for (unsigned index = 0; index < entries.size(); index++) {
                triples.emplace_back(nodes[index], "http://www.w3.org/1999/02/22-rdf-syntax-ns#first", entries[index],
                                     entryTypes[index], entrySubTypes[index]);
                triples.emplace_back(nodes[index], "http://www.w3.org/1999/02/22-rdf-syntax-ns#rest", nodes[index + 1],
                                     Type::Type_URI, "");
            }
            entry = nodes.front();
        }
    ...
    switch (token) {
            ...
            // std::string literal
        {
            ...
            if (token == Lexer::Token_At) {
                ...
                if (type == "http://www.w3.org/2001/XMLSchema#std::string") {
...
//---------------------------------------------------------------------------
void TurtleParser::parsePredicateObjectList(const std::string &subject, std::string &predicate, std::string &object,
                                            Type::Type_ID &objectType, std::string &objectSubType)
// Parse a predicate object list
{
    ...
    while (token == Lexer::Token_Comma) {
        std::string additionalObject, additionalObjectSubType;
        Type::Type_ID additionalObjectType;
        parseObject(additionalObject, additionalObjectType, additionalObjectSubType);
        triples.emplace_back(subject, predicate, additionalObject, additionalObjectType, additionalObjectSubType);
        token = lexer.next();
    }
    ...
    while (token == Lexer::Token_Semicolon) {
        ...
        Type::Type_ID additionalObjectType;
        parseObject(additionalObject, additionalObjectType, additionalObjectSubType);
        triples.emplace_back(subject, additionalPredicate, additionalObject, additionalObjectType,
...
//---------------------------------------------------------------------------
bool TurtleParser::parse(std::string &subject, std::string &predicate, std::string &object, Type::Type_ID &objectType,
